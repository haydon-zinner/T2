...
function multMat4Vec3(m, vx, vy, vz) {
	...
	for (var posId = 0; posId < 3; posId++) transformedVertex[posId] = dot(readVertex, m[posId]);
...
function ColladaParser(fileName) {
	...
	if (parser.bindShapeMatrix != null) colladaParser_TransformAllVertexesByBindShapeMatrix(this);
...
function colladaParser_MakeVertexDataCopy(parser) {
	...
	for (var vertexId = 0; vertexId < parser.vertexPositionDataRead.length; vertexId++) parser.vertexPositionDataWrite.push(parser.vertexPositionDataRead[vertexId]);
}
...
function colladaParser_TransformAllVertexesByBindShapeMatrix(parser) {
	...
	for (var vertexId = 0; vertexId < parser.vertexPositionDataRead.length / 3; vertexId++) {
		...
		for (var posId = 0; posId < 3; posId++) parser.vertexPositionDataRead[vertexId * 3 + posId] = transformedVertex[posId];
...
function colladaParser_BuildAnimations(parser, xmlDoc) {
	...
	if (animationLibraryList.length == 0) return;

	for (var animationTrackId = 0; animationTrackId < animationLibraryList.length; animationTrackId++) for (var boneId = 0; boneId < parser.bones.length; boneId++) parser.bones[boneId].animationTracks.push(new AnimationTrack());
...
function colladaParser_BuildScene(parser, xmlDoc) {
	...
	if (sceneNodeList.length == 0) return;
	...
	for (var temp = 0; temp < currentNodes.length; temp++) {
		if (currentNodes[temp].nodeType == 1) {
			...
			if (nodeElement.hasAttribute("type") && nodeElement.getAttribute("type") == "JOINT") {
				...
				} else setBoneBindPoseMatrix(parser, currentBone, nodeElement);
...
function getBone(parser, boneName) {
	for (var i = 0; i < parser.bones.length; i++) if (parser.bones[i].name == boneName) {
...
function getBoneId(parser, boneId) {
	...
	for (var i = 0; i < parser.tempBoneIds.length; i++) if (parser.tempBoneIds[i] == boneId) return parser.bones[i];
...
function collada_SubSet(arrayS, startIndex, size) {
	var ret = [];
	for (var i = 0; i < size; i++) ret.push(arrayS[startIndex + i]);
	return ret;
}
...
function colladaParser_BuildLibraryControllers(parser, xmlDoc) {
	...
	if (controllerNodeList.length == 0) return;
	parser.bones = [];
	parser.tempBoneIds = [];
	...
	if (controllerNodeList[0].nodeType == 1) {
		...
		if (nList.length > 0) {
			...
			//Source 2 Vertex Weights
			{
				...
				if (FloatArrayList.length > 0) vertexWeights = convertStringToFloatArrayList(FloatArrayList.item(0).firstChild.nodeValue);
			}

			var vCount = controllerElement.getElementsByTagName("vcount");
			if (vCount.length > 0) vertexCounts = convertStringToIntegerArrayList(vCount[0].firstChild.nodeValue);
			var v = controllerElement.getElementsByTagName("v");
			if (v.length > 0) vertexIndexes = convertStringToIntegerArrayList(v[0].firstChild.nodeValue);
			...
			if (vertexIndexes != null && vertexCounts != null && vertexWeights != null) {
				...
				for (var boneCountId = 0; boneCountId < vertexCounts.length; boneCountId++) {
					...
					for (var x = 0; x < numBones; x++) {
						if (x < 4) boneIndexes[x] = vertexIndexes[vOffSet];
						vOffSet++;
						if (x < 4) boneWeights[x] = vertexWeights[vertexIndexes[vOffSet]];
						vOffSet++;
					}
...
function fillVertexBoneData(parser, boneCountId, numBones, boneIndexes, boneWeights) {
	...
	for (var i = 0; i < parser.vertexIndexes.length; i++) if (parser.vertexIndexes[i] == boneCountId) {
...
function colladaParser_BuildGeometeryMeshNode(parser, geomMeshNode) {
	...
	for (var j = 0; j < numberSources; j++) {
		...
		var stride = 0;
		if (node.getElementsByTagName("accessor").length > 0) stride = parseInt(node.getElementsByTagName("accessor")[0].getAttribute("stride"));
		//console.log(stride);
		if (vertexPositions == null) {
			vertexPositions = convertStringToFloatArrayList(floatArrayString);
			for (var posId = 0; posId < vertexPositions.length; posId += 3) colladaParser_AddVertexPositionToBoundingBox(parser, vertexPositions[posId], vertexPositions[posId + 1], vertexPositions[posId + 2]);
	...
	if (triangleList.length == 0) triangleList = geomMeshNode.getElementsByTagName("polylist");
		...
		if (pList.length > 0) {
			...
			for (var index = 0; index < indexes.length; index++) {
				...
				if (count == 0) {
					...
					for (var x = 0; x < positionStride; x++) if (x < 3) parser.vertexPositionDataRead.push(vertexPositions[x + currentIndex * positionStride]);
				...
				if (count == 1) {
					for (var x = 0; x < normalStride; x++) if (x < 3) parser.vertexNormalDataRead.push(vertexNormals[x + currentIndex * normalStride]);
				}
				if (count == 2) {
					for (var x = 0; x < 2; x++)//use 2 instead of vertexTexture because we only want the first two coordinates (x,y)
						if (x < 2) parser.vertexTextureDataRead.push(vertexTexture[x + currentIndex * textureStride]);
				}
...
function convertStringToIntegerArrayList(line) {
	...
	for (var i = 0; i < splitLine.length; i++) numbers.push(parseInt(splitLine[i]));
	return numbers;
}
...
function convertStringToFloatArrayList(line) {
	var numbers = [];
	var splitLine = line.split(" ");
	for (var i = 0; i < splitLine.length; i++) numbers.push(parseFloat(splitLine[i]));
	return numbers;
}
...
function colladaParser_ImageLibrary(parser, xmlDoc) {
	...
	var node = imageNode.firstChild;
	for (var j = 0; j < xlen; j++) {
		if (node.nodeType == 1) colladaParser_ImageLibrary_GetImageName(parser, node);
		node = node.nextSibling;
	}
...
function colladaParser_ImageLibrary_GetImageName(parser, imageSubNode) {
	...
	for (var j = 0; j < xlen; j++) {
		if (node.nodeType == 1) parser.textureData.push([imageSubNode.getAttribute("id"), node.childNodes[0].data]);
		node = node.nextSibling;
	}
}